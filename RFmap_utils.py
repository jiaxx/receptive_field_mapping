...
def get_RF_map(FR, stim_table):
    ...
    elif 'x_position' in list(stim_table.keys()):
        x_locs = np.unique(stim_table['x_position'])
        y_locs = np.unique(stim_table['y_position'])
    else:
    ...
    for n in range(np.shape(FR)[0]):
        ...
        for i in range(np.shape(FR)[1]):
            ...
            if 'Pos_x' in list(stim_table.keys()):
                ...
                x_pos = np.where(x_locs==stim_table['x_position'].values[i])[0][0]
                y_pos = np.where(y_locs==stim_table['y_position'].values[i])[0][0]
            else:
                print('Column name mismatch')
        ...
        for idx_x, x in enumerate(x_locs):
            for idx_y, y in enumerate(y_locs):
                if 'Pos_x' in list(stim_table.keys()):
                    ...
                    rf_map_trial[idx_x, idx_y, :] = FR[n, np.where((stim_table['x_position'].values==x) & 
                                                               (stim_table['y_position'].values==y))[0]]
                else:
                    print('Column name mismatch')
